    ...
    public class Request_to_editController : Controller
    {
        ...
        public ActionResult Create(int? id, Request_to_edit request_to_edit, HttpPostedFileBase upload, HttpPostedFileBase uploadu)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("Details_for_client","staffs",new {id =staff.id });
            }
        ...
        {
            var con = db.contacts.FirstOrDefault();
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            ...
            var loginInfo = new NetworkCredential(con.control_email, con.Password_control);
            mail.From = new MailAddress("adenuni.mag@gmail.com");
            mail.To.Add(request_to_edit.Email);
            mail.Subject =" معلومات حسابك في جامعة عدن ";
            mail.IsBodyHtml = true;
            string body = "اسم المرسل: جامعة عدن "+"<br />" +
                            ":بريد المرسل"+con.control_email+"<b>" + "<br />" +"<b/>.عنوان الرسالة: معلومات حسابك في جامعة عدن " + "<br />"+  "<b>نص الرسالة: شكرا لتواصلك معنا " + request_to_edit.dgreeid +" : "+request_to_edit.fullName+ "<br/>يسرنا أنك أصبحت أحد مستخدمي موقع جامعة عدن" + " يمكنك أستخدام أسم المستخدم وكلمة المرور التاليتين لتعديل ملفك الشخصي على موقع الجامعة " + "<br />"+request_to_edit.staff.user_name+" : أسم المستخدم "+"<br/>"+ request_to_edit.staff.password+" : كلمة المرور" + "<br/>.تمنياتنا لك بالتوفيق" ;
            mail.Body = body;
